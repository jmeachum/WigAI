...
- [x] [AI-Review][HIGH] Align `set_selected_device_parameters` empty-array error code with docs/tests (`INVALID_PARAMETER` vs documented `MISSING_REQUIRED_PARAMETER`) or update docs to match behavior. [src/main/java/io/github/fabb/wigai/mcp/tool/DeviceParamTool.java:248]
...
- [x] [AI-Review][MEDIUM] Document `INVALID_PARAMETER` for `set_selected_device_parameters` when `parameters` is not an array or contains non-object entries (current implementation throws `IllegalArgumentException`). [src/main/java/io/github/fabb/wigai/mcp/tool/DeviceParamTool.java:251]
- [x] [AI-Review][MEDIUM] Update `BitwigApiFacadeTest` to stub `cursorTrack.position()` for `getSelectedDeviceInfo` (project-absolute index) so the new behavior is asserted. [src/test/java/io/github/fabb/wigai/bitwig/BitwigApiFacadeTest.java:470]
- [x] [AI-Review][MEDIUM] Align no-device-selected handling with AC2/docs; current unit test treats null device as success instead of `DEVICE_NOT_SELECTED`. [src/test/java/io/github/fabb/wigai/features/DeviceControllerTest.java:66]
- [x] [AI-Review][MEDIUM] Use `INVALID_RANGE` for out-of-range parameter values (update tests/behavior for single + batch). [src/test/java/io/github/fabb/wigai/features/DeviceControllerTest.java:158]
...
- Device: `get_selected_device_parameters`, `set_selected_device_parameter`, `set_selected_device_parameters`, `get_device_details`
...
- Story status: `Ready for Review` - all tasks and review follow-up items completed (final 5 items resolved 2025-12-30)
...
**2025-12-30 Final Review Follow-up (Session 5):**

33. **set_selected_device_parameters Empty Array Error Code Aligned** (HIGH priority):
    - Changed `DeviceParamTool.java` to throw `BitwigApiException` with `EMPTY_PARAMETER` instead of `IllegalArgumentException`
    - Semantically correct: `MISSING_REQUIRED_PARAMETER` = param not provided, `EMPTY_PARAMETER` = param provided but empty
    - Updated test `setSelectedDeviceParametersEmptyArrayError` in `BaselineToolEnvelopeAtddTest.java` to expect `EMPTY_PARAMETER`
    - Updated `docs/reference/api-reference.md` to document both `MISSING_REQUIRED_PARAMETER` (for missing array) and `EMPTY_PARAMETER` (for empty array)

34. **Test Evidence**: `./gradlew test` - All tests PASSED (2025-12-30) - final review follow-up item resolved

**2025-12-30 Correct-Course Resolution (Final 2 Items):**

35. **INVALID_PARAMETER Error Code for Malformed Parameters** (MEDIUM priority):
    - Changed `DeviceParamTool.java` to throw `BitwigApiException` with `INVALID_PARAMETER` instead of `IllegalArgumentException` for:
      - `parameters` not being an array
      - Parameter entries not being objects
    - Updated `docs/reference/api-reference.md` to document `INVALID_PARAMETER` in errors section

36. **BitwigApiFacadeTest cursorTrack.position() Stub** (MEDIUM priority):
    - Updated `testGetSelectedDeviceInfo_WithDeviceSelected` to explicitly stub `cursorTrack.position()` with value 5
    - Changed assertion from `assertEquals(0, result.get("track_index"))` to `assertEquals(5, result.get("track_index"))`
    - Ensures test actually validates project-absolute indexing behavior (not coincidentally passing with default 0)

37. **Error Code Semantics Added to project-context.md** (Preventive):
    - Added "Error Code Semantics" section to `docs/project-context.md` with canonical definitions
    - Prevents future review cycles caused by unclear error code taxonomy
    - Definitions: MISSING_REQUIRED_PARAMETER, EMPTY_PARAMETER, INVALID_PARAMETER, INVALID_PARAMETER_INDEX, INVALID_RANGE, BITWIG_API_ERROR, INTERNAL_ERROR

38. **Test Evidence**: `./gradlew test` - All tests PASSED (2025-12-30) - all review follow-up items resolved

**2025-12-30 Final Review Follow-ups (Session 6):**

39. **No-Device-Selected Test Fixed** (MEDIUM priority):
    - Updated `DeviceControllerTest.testGetSelectedDeviceParameters_NoDevice` to expect `DEVICE_NOT_SELECTED` error
    - Test now correctly mocks `BitwigApiFacade.getSelectedDeviceName()` throwing exception when no device is selected
    - Aligns with AC2/docs: error responses must include proper error code

40. **INVALID_RANGE for Out-of-Range Values** (MEDIUM priority):
    - Updated `testSetSelectedDeviceParameter_ValueValidationError` to use `INVALID_RANGE` instead of `INVALID_PARAMETER`
    - Updated `testSetMultipleSelectedDeviceParameters_PartialSuccess` and `testSetMultipleSelectedDeviceParameters_AllValidationErrors` batch tests
    - Aligns with project-context.md: `INVALID_RANGE` is for numeric values outside allowed range (0.0-1.0)

41. **remote_controls[].exists Docs Aligned** (MEDIUM priority):
    - Updated `docs/reference/api-reference.md` line 768: `exists` is always `true` since only existing parameters are included
    - Removed incorrect heuristic about name being non-empty

42. **Story Status Mismatch Resolved** (LOW priority):
    - Updated Story Completion Status to accurately reflect current state
    - All review follow-up items now marked [x] complete

43. **Baseline Tool List Typo Fixed** (LOW priority):

...
- [ ] [AI-Review][MEDIUM] Align no-device-selected handling with AC2/docs; current unit test treats null device as success instead of `DEVICE_NOT_SELECTED`. [src/test/java/io/github/fabb/wigai/features/DeviceControllerTest.java:66]
- [ ] [AI-Review][MEDIUM] Use `INVALID_RANGE` for out-of-range parameter values (update tests/behavior for single + batch). [src/test/java/io/github/fabb/wigai/features/DeviceControllerTest.java:158]
- [ ] [AI-Review][MEDIUM] Align `get_device_details` `remote_controls[].exists` docs with implementation (impl always true; docs say false when name empty). [docs/reference/api-reference.md:768]
- [ ] [AI-Review][LOW] Resolve story status mismatch (header + completion status vs sprint status). [docs/sprint-artifacts/1-3-standardize-baseline-tool-response-envelopes-align-with-status-tool-api-reference.md:3]
- [ ] [AI-Review][LOW] Fix baseline tool list entry to `set_selected_device_parameters`. [docs/sprint-artifacts/1-3-standardize-baseline-tool-response-envelopes-align-with-status-tool-api-reference.md:105]
...
- Story status: `Ready for Review` - all tasks and review follow-up items completed (final 2 items resolved via correct-course 2025-12-30)

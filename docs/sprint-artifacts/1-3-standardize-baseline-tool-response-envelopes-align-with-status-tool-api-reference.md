...
- [ ] [AI-Review][MEDIUM] Document `INVALID_PARAMETER` for `set_selected_device_parameters` when `parameters` is not an array or contains non-object entries (current implementation throws `IllegalArgumentException`). [src/main/java/io/github/fabb/wigai/mcp/tool/DeviceParamTool.java:251]
- [ ] [AI-Review][MEDIUM] Update `BitwigApiFacadeTest` to stub `cursorTrack.position()` for `getSelectedDeviceInfo` (project-absolute index) so the new behavior is asserted. [src/test/java/io/github/fabb/wigai/bitwig/BitwigApiFacadeTest.java:470]
...
- Story status: `Ready for Review` - all tasks and review follow-up items completed (final item 2025-12-30)
